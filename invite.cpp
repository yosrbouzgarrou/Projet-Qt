...
QSqlQueryModel * invite::rechercher22(const QString& XXX)
{

    QSqlQueryModel * model= new QSqlQueryModel();

model->setQuery("select * from invite WHERE ((ID_INVITE || NOM_PRENOM || EMAIL || EMAIL) LIKE '%"+XXX+"%')");
...
QSqlQueryModel * invite::trierr(const QString &critere, const QString &mode )
{
    QSqlQueryModel * model= new QSqlQueryModel();

model->setQuery("select * from invite order by "+critere+" "+mode+"");
model->setHeaderData(0, Qt::Horizontal, QObject::tr("ID_INVITE"));
model->setHeaderData(1, Qt::Horizontal, QObject::tr("NOM_PRENOM"));
model->setHeaderData(2, Qt::Horizontal, QObject::tr("EMAIL"));
...
QSqlQueryModel * invite::afficherid(){
    QSqlQueryModel * model= new QSqlQueryModel();
    model->setQuery("select ID_INVITE  from INVITE");
    model->setHeaderData(0, Qt::Horizontal, QObject::tr("etat"));
    return model;
}
QSqlQueryModel * invite::affichermail(){
    QSqlQueryModel * model= new QSqlQueryModel();
    model->setQuery("select EMAIL  from INVITE");
    model->setHeaderData(0, Qt::Horizontal, QObject::tr("etat"));
    return model;
}
