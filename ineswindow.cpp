...
void MainWindow::on_pushButton_14_clicked()//AJOUTER MATERIEL
{
    ...
    if(test)
    {
        ...
        n->show();
        //mysystem->showMessage(tr("Notificationn"),tr("Il y a un ajout d'un Materiel "));
    ...
    int code= ui->lineEdit_code->text().toInt();
        ...
        if(test)
        {
            n->setPopupText("Materiel  a été supprimé");
                        n->show();
                        mysystem->showMessage(tr("Notificationn"),tr("Il y a une supression d'un Materiel "));
     ...
     bool test=tempmateriel.modifier_materiel( code );
...

        {
            n->setPopupText("Materiel  a été modifié");
                        n->show();
                        mysystem->showMessage(tr("Notificationn"),tr("Il y a une modification d'un Materiel "));
    ...
    bool test;
        ...
        fourniture f(ref,nom,loc,type);
         ...
         { n->setPopupText("Fourniture  a été ajouté");
             n->show();
            // mysystem->showMessage(tr("Notificationn"),tr("Il y a un ajout d'un Fourniture "));
  ...
  bool test=tmpfourniture. modifier( ref,loc );
     if (test)
     {
         n->setPopupText("Fourniture  a été modifié");
                      n->show();
                      mysystem->showMessage(tr("Notificationn"),tr("Il y a une modification d'un Fourniture "));
    ...
    int ref= ui->lineEdit_ref_2->text().toInt();
        ...
        if(test)
        {
            n->setPopupText("Fourniture  a été supprimé");
                         n->show();
                         mysystem->showMessage(tr("Notificationn"),tr("Il y a une supression d'un Fourniture "));
...
void MainWindow::on_radioButton_clicked()//trie
{
...
void MainWindow::on_pushButton_10_clicked()//recherche
{
    int ref;
    ref=ui->lineEdit->text().toInt();
...
void MainWindow::on_radioButton_2_clicked()//trie
{
...
void MainWindow::on_tableView_2_activated(const QModelIndex &index)//affichage
{
...
void MainWindow::on_PDF_matr_clicked()//pdf
{
    QPdfWriter pdf("C:/Users/user/Music/SMARTNEWSAGENCY_INESHOSNI_2A15/materiel.pdf");
                  QPainter painter(&pdf);
                 int i = 4000;
                      painter.setPen(Qt::blue);
                      painter.setFont(QFont("Arial", 25));
                      painter.drawText(950,1100,"Liste Des materiels");
                      painter.setPen(Qt::black);
                      painter.setFont(QFont("Arial", 15));
                     // painter.drawText(1100,2000,afficheDC);
                      painter.drawRect(100,100,7300,2600);
                      ...
                      painter.drawText(2100,3300,"NOM");
                      painter.drawText(3200,3300,"TYPE");
                      painter.drawText(5300,3300,"QUANTITE");



  QSqlQuery query;
                      query.prepare("select * from GESTIONMATERIEL");
                      query.exec();
                      while (query.next())
                      {
